...
class Bottleneck(nn.Module):
    ...
    def __init__(self, inplanes, planes, with_ibn=False, with_se=False, gcb=None, stride=1, downsample=None,
        ...
        if with_ibn:
            ...
            self.bn1 = FRN(planes)
        self.relu1 = TLU(planes)
        self.conv2 = nn.Conv2d(planes, planes, kernel_size=3, stride=stride,
                               padding=1, bias=False)
        # self.bn2 = nn.BatchNorm2d(planes)
        self.bn2 = FRN(planes)
        self.relu2 = TLU(planes)
        self.conv3 = nn.Conv2d(planes, planes * 4, kernel_size=1, bias=False)
        # self.bn3 = nn.BatchNorm2d(planes * 4)
        self.bn3 = FRN(planes * 4)
        self.relu3 = TLU(planes * 4)
...
class ResNetFRN(nn.Module):
    ...
    def from_name(cls, model_name, pretrained, last_stride, with_ibn, with_se, gcb, stage_with_gcb, model_path):
        ...
        return ResNetFRN(pretrained, last_stride, with_ibn, with_se, gcb, stage_with_gcb,
